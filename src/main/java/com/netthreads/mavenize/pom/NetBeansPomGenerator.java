...
/**
 * The "NetBeans" module pom generator has added ability to read the project definition 
 * files and make a stab at dependencies.
 * 
 * @author Alistair
 */
public class NetBeansPomGenerator extends DefaultPomGenerator
{
    ...
    /**
     * When trying to generate the appropriate groupid and artifactid for a
     * dependency we are going to split the "code-name-base" from the project.xml
     * in two along this division from the right to left. So something like
     * "org.gephi.project.api" splits into groupid=org.gephi and artifactid=project.api.
     * Obviously this isn't going to work in every case but it's a start.
     */
    ...
    private void populateBuild(Model model)
    {
        ...
        pluginCompiler.setGroupId("org.apache.maven.plugins"); // TODO Bug in API?
    ...
    private List<Dependency> load(String path)
    {
        ...
        try
        {
            ...
            dependencies = project.getConfiguration().getData().getModuleDependencies().getDependency();
    ...
    /**
     * Returns true if codeNameBase has starting component to package name which 
     * matches one of those loaded from properties.
     * 
     * @param codeNameBase
     * 
